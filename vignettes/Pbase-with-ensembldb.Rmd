...
```{r}
## What data do I need:
## o ALL from protein table
## o NOTHING from exon or tx2exon table(s)
## o Optionally the protein_domain table (but if, then load all).
## o Optionally the uniprot table.
## Parameter 'columns' should not contain any columns from exon, tx2exon or
## protein_domain tables.

## That's now more to check how we could get the data and how we could implement
## the Proteins method.
prt <- proteins(edb, filter = GenenameFilter("ZBTB16"),
                return.type = "data.frame", columns = listProteinColumns(edb))
## Without list and lapply:
pids <- unique(prt$protein_id)
## If loadProteinDomains == TRUE
prt_dom_cols <- listColumns(edb, "protein_domain")
prt_dom_mcols <- prt_dom_cols[!(prt_dom_cols %in% c("protein_domain_id",
                                                     "prot_dom_start",
                                                     "prot_dom_end"))]
prot_dom <- unique(prt[, prt_dom_cols, drop = FALSE])
## Get rid of empty ones.
prot_dom <- prot_dom[!is.na(prot_dom$prot_dom_start), , drop = FALSE]
## Make the IRangesList
if (nrow(prot_dom) > 0) {
    prng <- IRanges(start = prot_dom$prot_dom_start, end = prot_dom$prot_dom_end)
    names(prng) <- prot_dom$protein_domain_id
    mcols(prng) <- prot_dom[, prt_dom_mcols]
    prng <- split(prng, f = factor(prot_dom$protein_id,
                                   levels = unique(prot_dom$protein_id)))
}
## Fill the empty ones.
if (!all(pids %in% names(prng))) {
    miss_prt <- pids[!(pids %in% names(prng))]
...
## Fix ordering
prng <- prng[pids]
## Make the AAStringSet.
## Get the unique data for each protein: all that is NOT in protein_domain
prt_cn <- colnames(prt)[!(colnames(prt) %in%
                          listColumns(edb, c("protein_domain", "uniprot")))]
prt <- unique(prt[, c("protein_id", prt_cn)])
aa <- AAStringSet(prt$protein_sequence)
names(aa) <- prt$protein_id
mcols(aa) <- prt[, !(colnames(prt) %in% c("protein_id", "protein_sequence")),
                 drop = FALSE]
## Create the Proteins object.
mtd <- list(created = date(), ensembl_version = ensemblVersion(edb))
pr <- new("Proteins", aa = aa, pranges = prng, metadata = mtd)
