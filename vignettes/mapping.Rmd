...
Below, we store the transcripts and protein identifiers for later use.
...
## the following does not recover the expected gene anymore,
## possible due to differences in coordinates between Ensembl and USCS
## bmTrack <- BiomartGeneRegionTrack(start = min(bm$start_position),
##                                   end = max(bm$end_position),
##                                   chromosome = "X",
##                                   genome = "hg19")

## setting start/end positions manually for now
bmTrack <- BiomartGeneRegionTrack(start = 77359666,
                                  end = 77382324,
                                  chromosome = "X",
                                  genome = "hg19")
...
```{r, txdb, cache=FALSE, message=FALSE, eval=FALSE}
...
txdb <- TxDb.Hsapiens.UCSC.hg19.knownGene
txTr <- GeneRegionTrack(txdb, chromosome = "chrX")
tmp <- as(txTr, "data.frame")
s1 <- min(tmp[which(tmp$gene == "5230"), "X.start"])
s2 <- max(tmp[which(tmp$gene == "5230"), "X.end"])
```

Note however that the `biomart` (from Ensembl) and `TranscriptDb`
(from USSC) start and end positions might not match (above, for
example worked up to Bioconductor release 3.0). Below, we use
positions `s1` `r s1` and `s2` `r s2`, directly extracted for the
`uc004ecz.4` transcript from `txdb` (see below to query the UCSC
transcript identifier).


```{r, txdb2, cache=TRUE, message=FALSE}
library("TxDb.Hsapiens.UCSC.hg19.knownGene")
txdb <- TxDb.Hsapiens.UCSC.hg19.knownGene
txTr <- GeneRegionTrack(txdb, chromosome = "chrX",
                        start = s1, end = s2)
txTr <- txTr[transcript(txTr) %in% "uc004ecz.4"]
...
It is also possible to use package genomes to do this. Above, we have
used the `TxDb.Hsapiens.UCSC.hg19.knownGene` package defining
...
UCSC equivalent. (**TODO** Use also `Homo.sapiens` annotation
package.)
...
plotTracks(list(ideoTrack, axisTrack, txTr, pepTr),
           groupAnnotation = "id",
           just.group = "below",
           fontsize.group = 9,
           add53 = TRUE, add35 = TRUE)
...
plotTracks(list(ideoTrack, axisTrack, deTrack, txTr),
           add53 = TRUE, add35 = TRUE)
```
