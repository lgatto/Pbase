...
Ensembl Biomart server for `r seqnames(p)[1]`, we obtain several
...
Let's fetch the coordinates of all possible transcipts and making sure
that the names of the Ensembl identifiers are used to name the `grl`
ranges (using `use.names = TRUE`). We obtain `r nrow(bm)` sets of
ranges for `r length(p)` proteins.
...
As we can see on the next figures, only `r bm[3:4, "ensembl_transcript_id"]`
(including the actual transcript) show comparable exons patterns.

```{r plot5}
grTr <- lapply(1:5, function(i)
    GeneRegionTrack(pcgrl[[i]],
                    name = bm[i, "ensembl_transcript_id"]))
plotTracks(grTr)
```

Using `r seqnames(p)[1]` as an example, we will use `mapToGenome` to
map these peptides against all possible transcript exons.

```{r map2}
res <- mapToGenome(p[1], pcgrl)
```

As can be seen above, 3 warnings are throw because the mapping
failed. Such failed mapping produce empty ranges that are
automatically dropped from the result, leaving `r length(res)`
relevant ranges in the `res` output.

```{r pepcoords2}
pepTr <- lapply(res, function(.res)
    AnnotationTrack(start = start(.res),
                    end = end(.res),
                    chr = chrom(.res),
                    strand = strand(.res),
                    id = mcols(.res)$pepseq,
...
plotTracks(list(grTr[[3]], pepTr[[1]],
                grTr[[4]], pepTr[[2]]))
```

The `r length(res)` transcript are indistinguishable based on the
mapped peptides (`r elementLengths(res)[1]` and
`r elementLengths(res)[2]`, respectively). Below, we further confirm this
by aligning the coding sequences of the two transcripts.

```{r getseq2, warning=FALSE}
s1 <- translate(unlist(getSeq(BSgenome.Hsapiens.NCBI.GRCh38, pcgrl[[3]])))
s2 <- translate(unlist(getSeq(BSgenome.Hsapiens.NCBI.GRCh38, pcgrl[[4]])))
writePairwiseAlignments(pairwiseAlignment(s1, s2))
```

We can apply the above many-to-one mapping approach to all proteins in
