...
Below, we create a `Mart` object that stores the connection details to
the latest human Ensembl biomart server.
...
```{r compgr, echo=FALSE}
data.frame(feature = feature(grTrack),
           start = start(grTrack) - start(grTrack2),
           end = end(grTrack) - end(grTrack2))
```

More substantial differences may arise if UCSC or Ensembl are based on
different genome references. See the `Mapping between Ensembl and
UCSC` vignette in `Pbase` for details:

```{r eval=FALSE}
vignette("ensucsc", package = "Pbase")
```
...
with regions and features). As above, we have used the `r
Biocannopkg("TxDb.Hsapiens.UCSC.hg38.knownGene")` package to identify
genomic coordinates, we want to use a genome package that is based on
the same reference genome, namely `r
Biocannopkg("BSgenome.Hsapiens.NCBI.GRCh38")`.

```{r protfromgenome}
library("BSgenome.Hsapiens.NCBI.GRCh38")
seqlevels(prng) <- "X"
genome(prng) <- "GRCh38"
s <- getSeq(BSgenome.Hsapiens.NCBI.GRCh38, prng)
...
2) Calculate the peptide coordinates on the cDNA sequence `peprng2`
